...
class HandState:
    def __init__(self, frame, hand_landmarks):
        ...
        self.FOCAL_LENGTH = 1.0  # Assuming normalized coordinates where focal length is 1
        ### === Camera information === ###
        self.H = self.frame.shape[0] # Height
        self.W = self.frame.shape[1] # Width
        self.D = self.frame.shape[2] # Depth
...
### === Helper Functions === ###
    ...
    def project_point(self,point3D, cx, cy):
        ...
        u = int((x * self.FOCAL_LENGTH / abs(z)) + cx)
        v = int((y * self.FOCAL_LENGTH / abs(z)) + cy)
        return (u, v)

    def to_pixel_coords(self, lm):
        # self.landmarks = hand_landmarks#.landmark
        return int(lm.x * self.W), int(lm.y * self.H), int(lm.z * self.D)
...
### === Finger State Function -- Extended, Folded or inbetween === ###    
    def finger_flexion(self, finger_type, th_low=50, th_high=120):
        ...
        curl = 0.0
        folded = False
        wrist_vect = self.get_landmark_vector(self.landmarks[self.WRIST_idx])


        if finger_type == 'THUMB':
            thumb_tip_vect = self.get_landmark_vector(self.landmarks[self.FINGERS["tip_idx"][0]])  # Thumb tip
            pinky_base_vect = self.get_landmark_vector(self.landmarks[self.FINGERS["base_idx"][4]])  # Pinky base
            thumb_pip_vect = self.get_landmark_vector(self.landmarks[self.FINGERS["pip_idx"][0]])  # Thumb pip
            ...
            v1 = self.get_landmark_vector(self.landmarks[self.FINGERS["pip_idx"][self.FINGERS["name"].index(finger_type)]]) - self.get_landmark_vector(self.landmarks[self.FINGERS["base_idx"][self.FINGERS["name"].index(finger_type)]])
            v2 = self.get_landmark_vector(self.landmarks[self.FINGERS["dip_idx"][self.FINGERS["name"].index(finger_type)]]) - self.get_landmark_vector(self.landmarks[self.FINGERS["pip_idx"][self.FINGERS["name"].index(finger_type)]])
            curl += self.vector_angle(v1, v2) # because computing based on angle sum

            # angle 2: pip->dip and dip->tip
            v3 = self.get_landmark_vector(self.landmarks[self.FINGERS["tip_idx"][self.FINGERS["name"].index(finger_type)]]) - self.get_landmark_vector(self.landmarks[self.FINGERS["dip_idx"][self.FINGERS["name"].index(finger_type)]])
            curl += self.vector_angle(v2, v3)

            # Distance based condition
            tip_vect = self.get_landmark_vector(self.landmarks[self.FINGERS["tip_idx"][self.FINGERS["name"].index(finger_type)]]) 
            pip_vect = self.get_landmark_vector(self.landmarks[self.FINGERS["pip_idx"][self.FINGERS["name"].index(finger_type)]])
    ...
    # different thresholds from finger_flexion function => useless
       ...
       -1  -> Folded
        """
        finger_types = self.FINGERS["name"]
        finger_state = []
...
### === Hand Orientation to camera & Position to center(?) === ###
    def hand_orientation(self, frame, hand_label, th=45):
        ...
        palm_vec1 = self.get_landmark_vector(self.landmarks[self.FINGERS["base_idx"][4]]) - self.get_landmark_vector(self.landmarks[self.FINGERS["base_idx"][1]])
        # Palm vector 2: wrist to Middle base
        palm_vec2 = self.get_landmark_vector(self.landmarks[self.WRIST_idx]) - self.get_landmark_vector(self.landmarks[self.FINGERS["base_idx"][2]])
        ...
        wrist_px = (int(self.landmarks[self.WRIST_idx].x * self.W), int(self.landmarks[self.WRIST_idx].y * self.H))
        ...
        dot = np.dot(palm_normal, self.global_vars.camera_view)
    ...
    def hand_position(self):
        ...
        centerHand = self.landmarks[self.WRIST_idx]
...
### === Additional hand state rules for symbolic representation -- too see if will be used === ###
    ...
    def thumb_direction(self, is_thumb_straight, th=40):
        ...
        thumb_vec = self.get_landmark_vector(self.landmarks[self.global_vars.FINGERS["tip_idx"][0]]) - self.get_landmark_vector(self.landmarks[self.global_vars.FINGERS["pip_idx"][0]])
        ...
        # Do your hand logic here (e.g., compute vectors, draw cross product, etc.)
            ...
            if results.multi_hand_landmarks:
                ...
                for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                    ...
                    handsClass = HandState(frame_rgb, hand_landmarks)
